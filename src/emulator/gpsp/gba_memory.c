...
#define GBA_ROM_ADAPTIVE_PREFETCH 1

u32 gba_rom_page_loads;
u32 gba_rom_page_prefetches;
#endif
...
#define open_bus_address_readable(addr)                                       \
  (((addr) >> 24) <= 0x0F)

#define open_bus_value8(addr)                                                 \
  ((u8)(reg[REG_BUS_VALUE] >> (((addr) & 0x03) << 3)))

#define open_bus_value16(addr)                                                \
  ((u16)(reg[REG_BUS_VALUE] >> (((addr) & 0x02) << 3)))

#define open_bus_value32(addr)                                                \
  (reg[REG_BUS_VALUE])

#define read_open8()                                                          \
  {                                                                           \
    u32 open_addr = (!(reg[REG_CPSR] & 0x20)) ?                               \
      (reg[REG_PC] + 8 + (address & 0x03)) :                                  \
      (reg[REG_PC] + 4 + (address & 0x01));                                   \
    value = open_bus_address_readable(open_addr) ?                            \
      read_memory8(open_addr) : open_bus_value8(address);                     \
  }                                                                           \

#define read_open16()                                                         \
  {                                                                           \
    u32 open_addr = (!(reg[REG_CPSR] & 0x20)) ?                               \
      (reg[REG_PC] + 8 + (address & 0x02)) :                                  \
      (reg[REG_PC] + 4);                                                      \
    value = open_bus_address_readable(open_addr) ?                            \
      read_memory16(open_addr) : open_bus_value16(address);                   \
  }                                                                           \

#define read_open32()                                                         \
  {                                                                           \
    u32 open_addr = (!(reg[REG_CPSR] & 0x20)) ?                               \
      (reg[REG_PC] + 8) : (reg[REG_PC] + 4);                                  \
    if(open_bus_address_readable(open_addr))                                  \
    {                                                                         \
      if(!(reg[REG_CPSR] & 0x20))                                             \
        value = read_memory32(open_addr);                                     \
      else                                                                    \
      {                                                                       \
        u32 current_instruction = read_memory16(open_addr);                   \
        value = current_instruction | (current_instruction << 16);            \
      }                                                                       \
    }                                                                         \
    else                                                                      \
      value = open_bus_value32(address);                                      \
  }                                                                           \
...
#define read_memory(type)                                                     \
  ...
  {                                                                           \
    ...
    case 0x03:                                                                \
      /* internal work RAM */                                                 \
      value = readaddress##type(iwram, (address & 0x7FFF) +                 \
          GBA_IWRAM_DATA_OFFSET);                                           \
      break;                                                                  \
                                                                              \
    case 0x04:                                                                \
      /* I/O registers */                                                     \
...
#define write_memory(type)                                                    \
  ...
  {                                                                           \
    ...
    case 0x03:                                                                \
      /* internal work RAM */                                                 \
      address##type(iwram, (address & 0x7FFF) +                             \
          GBA_IWRAM_DATA_OFFSET) = eswap##type(value);                       \
      break;                                                                  \
                                                                              \
    case 0x04:                                                                \
      /* I/O registers */                                                     \
...
#define dma_read_iwram(type, tfsize)                                          \
  read_value = readaddress##tfsize(iwram + GBA_IWRAM_DATA_OFFSET,             \
      type##_ptr & 0x7FFF)                                                    \
...
#ifdef HAVE_DYNAREC
#define dma_check_iwram_smc(type, tfsize)                                     \
...
#define dma_write_iwram(type, tfsize)                                         \
  address##tfsize(iwram + GBA_IWRAM_DATA_OFFSET, type##_ptr & 0x7FFF) =       \
                                          eswap##tfsize(read_value);          \
  dma_check_iwram_smc(type, tfsize);                                          \
...
#ifdef HAVE_DYNAREC
#define dma_check_ewram_smc(type, tfsize)                                     \
...
#define dma_write_ewram(type, tfsize)                                         \
  ...
  dma_check_ewram_smc(type, tfsize);                                          \
...
static u8 *load_gamepak_page_uncached(u32 physical_index, bool is_prefetch)
{
...
#if defined(RETRO_GO) && GBA_ROM_ADAPTIVE_PREFETCH
static inline bool gamepak_page_is_cached(u32 physical_index, u32 rom_pages)
{
  if(physical_index >= rom_pages)
    return true;

  return memory_map_read[(0x8000000 >> 15) + physical_index] != NULL;
}

static inline void prefetch_gamepak_page(u32 physical_index, u32 rom_pages)
{
  if(!gamepak_page_is_cached(physical_index, rom_pages))
    load_gamepak_page_uncached(physical_index, true);
}

static void prefetch_gamepak_pages_after_miss(u32 physical_index)
{
  static u32 last_miss_index = 0xFFFFFFFF;
  static s32 last_stride = 0;
  static u8 stream_misses = 0;
  static u32 budget_base_loads = 0;
  static u32 budget_base_prefetches = 0;

  if(gamepak_buffer_count < 6)
    return;
  ...
  s32 stride = 0;
  if(last_miss_index != 0xFFFFFFFF)
    stride = (s32)physical_index - (s32)last_miss_index;

  if(stride == last_stride && (stride == 1 || stride == -1))
  {
    if(stream_misses < 4)
      stream_misses++;
  }
  ...
  {
    stream_misses = (stride == 1 || stride == -1) ? 1 : 0;
  }

  last_miss_index = physical_index;
  last_stride = stride;

  u32 real_loads = gba_rom_page_loads - budget_base_loads;
  u32 prefetches = gba_rom_page_prefetches - budget_base_prefetches;
  if(real_loads >= 96)
  {
    budget_base_loads = gba_rom_page_loads;
    budget_base_prefetches = gba_rom_page_prefetches;
    real_loads = 0;
    prefetches = 0;
  }

  // ROM prefetch is synchronous SD I/O, so keep it useful but bounded. This
  // allows a little lookahead during scene streams without doubling SD traffic.
  if(prefetches >= (real_loads / 3 + 1))
    return;

  if(stride == 1 || stream_misses >= 2 || stride > 3)
    prefetch_gamepak_page(physical_index + 1, rom_pages);
}
#endif

u8 *load_gamepak_page(u32 physical_index)
{
  u8 *swap_location = load_gamepak_page_uncached(physical_index, false);

#if defined(RETRO_GO) && GBA_ROM_ADAPTIVE_PREFETCH
  prefetch_gamepak_pages_after_miss(physical_index);
#endif
...
void init_gamepak_buffer(void)
{
  ...
  {
#ifdef RETRO_GO
    void *ptr = rg_alloc(gamepak_buffer_blocksize, MEM_SLOW | MEM_8BIT | MEM_NOPANIC);
    if (!ptr)
      ptr = rg_alloc(gamepak_buffer_blocksize, MEM_ANY | MEM_NOPANIC);
#else
...
#ifdef RETRO_GO
  RG_LOGI("GBA ROM cache: %u MB", gamepak_buffer_count);
#endif
...
void init_memory(void)
{
  ...
  map_region(read, 0x3000000, 0x4000000, 1,
...
bool memory_read_savestate(const u8 *src)
{
  ...
  if (!(
    bson_read_bytes(memdoc, "iwram", &iwram[GBA_IWRAM_DATA_OFFSET],
      GBA_IWRAM_DATA_SIZE) &&
...
unsigned memory_write_savestate(u8 *dst)
{
  ...
  bson_write_bytes(dst, "iwram", &iwram[GBA_IWRAM_DATA_OFFSET],
      GBA_IWRAM_DATA_SIZE);
...
static s32 load_gamepak_raw(const char *name)
{
  ...
  if(gamepak_file_large)
  {
    ...
    for (i = 0; i < ldblks; i++)
    {
      ...
      size_t read_count = fread(gamepak_buffers[i], gamepak_buffer_blocksize, 1, gamepak_file_large);
      if (read_count != 1)
        return -1;
