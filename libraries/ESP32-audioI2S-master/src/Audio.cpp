...
Audio::Audio(uint8_t i2sPort) {
    ...
    // DOLL-OS local patch: was 32. The ring is DMA-capable *internal* RAM, the scarce pool
    // on this board; 16 x 222 frames is still ~80ms at 44.1kHz, double the ~38ms panel-push
    // hole that sets the floor. Reverts on a library update -- re-apply if audio starts
    // stuttering after one.
