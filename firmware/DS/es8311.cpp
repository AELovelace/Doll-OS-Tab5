...
/*
 * Register access goes through Wire instead of the original legacy
 * driver/i2c.h calls: LovyanGFX links Wire, whose arduino-esp32 3.x backend
 * is the new i2c_master ("driver_ng") driver, and ESP-IDF aborts at boot if
 * both I2C drivers end up linked into the same binary. The sketch calls
 * Wire.begin() before es8311_codec_init(); es->port is kept but unused.
 */
...
static inline esp_err_t es8311_read_reg(es8311_handle_t dev, uint8_t reg_addr, uint8_t *reg_value)
{
    ...
    if (Wire.requestFrom((uint16_t) es->dev_addr, (size_t) 1, true) != 1) {
        return ESP_FAIL;
    }
...
{
    /* The codec's CE strap picks its I2C address (0x18 CE-low / 0x19 CE-high) -- probe
     * both instead of assuming 0x18. And fail *softly*: this used to wrap es8311_init in
     * ESP_ERROR_CHECK, which abort()s the whole firmware on a NACK; the caller
     * (radioEnsureCodec, Radio.ino) handles a non-OK return by reporting to the shell
     * and leaving everything else running. */
    uint16_t dev_addr = 0;
    const uint16_t candidates[] = { ES8311_ADDRESS_0, ES8311_ADDRESS_1 };
    esp_log_level_set("i2c.master", ESP_LOG_NONE);   //a NACK on the absent candidate is *expected*
                                                      //here -- keep the driver from logging it as an error
    for (int i = 0; i < 2; i++) {
        Wire.beginTransmission((uint8_t) candidates[i]);
        if (Wire.endTransmission() == 0) {
            dev_addr = candidates[i];
            break;
        }
    }
    esp_log_level_set("i2c.master", ESP_LOG_ERROR);
    ESP_RETURN_ON_FALSE(dev_addr != 0, ESP_FAIL, TAG, "no ES8311 ACK at 0x18 or 0x19");

    es8311_handle_t es_handle = es8311_create(I2C_NUM_0, dev_addr);
    ...
    if (err == ESP_OK) {
        err = es8311_sample_frequency_config(es_handle, EXAMPLE_SAMPLE_RATE * EXAMPLE_MCLK_MULTIPLE, EXAMPLE_SAMPLE_RATE);
    }
    if (err == ESP_OK) {
        err = es8311_voice_volume_set(es_handle, EXAMPLE_VOICE_VOLUME, NULL);
    }
    //if (err == ESP_OK) { err = es8311_microphone_config(es_handle, false); }
    if (err != ESP_OK) {
        free(es_handle);   //failed bring-up may be retried (next "radio play") -- don't leak the handle
        ESP_RETURN_ON_ERROR(err, TAG, "es8311 bring-up failed (addr 0x%02x)", dev_addr);
    }
    return ESP_OK;
}
