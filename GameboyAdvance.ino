...
static void gbaRunBootSession() {
    ...
    for (;;) {
        ...
        if (perfFrames >= perfTargetFrames) {
            ...
            const uint32_t jitWordStoreSpecialized =
                coreStats.jit_word_store_specialized -
                modeStart.jit_word_store_specialized;
...
#if DOLL_GBA_VERBOSE_DIAGNOSTICS
            Serial.printf("[gba perf] mode=%dx skip=%d emu=%lu.%lu drawn=%lu.%lu core=%lluus drawcore=%lluus skipcore=%lluus audio=%lluus blit=%lluus arm/thumb/halt=%lu/%lu/%lu pc=%08lx cpsr=%08lx jit=%lu/%luK hit/miss/try=%lu/%lu/%lu ops=%lu build=%lu full=%lu reuse=%lu wait/reject/probe=%lu/%lu/%lu break=%02lx:%lu spec=%lu/%lu/%lu batch=%lu/%lu fast=%lu/%lu predrop=q/set/dup:%lu/%lu/%lu prechurn=evict/stall:%lu/%lu preprobe=1/2/3/4/m:%lu/%lu/%lu/%lu/%lu preocc=now/cap/high:%lu/%lu/%lu vram=%s rom=%lu+%lu pace_resync=%lu cpu=%luMHz\n",
                          gbaScale,
                          gbaFrameSkip,
